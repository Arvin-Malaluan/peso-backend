...
class Post(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.image:
            ...
            content = base64.b64encode(self.image.read())
            # Construct GitHub file name
            timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
            new_file_name = f'community/{self.profile.name.replace(" ", "_")}_photo_{timestamp}.jpg'  # Example file extension (.jpg)
